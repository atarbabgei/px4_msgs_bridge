...
void Px4ToRosConverter::try_publish_pose()
{
    ...
    // ROBUST RATE LIMITING: Use ROS system time (not PX4 timestamps)
...
sensor_msgs::msg::Imu Px4ToRosConverter::convert_vehicle_imu()
{
    ...
    imu_msg.header.stamp = rclcpp::Clock().now();  // DDS-synchronized ROS time
...
{
    // SIMPLEST APPROACH: Just like the original working version
...
{
    // Convert from NED to your custom coordinate frame (NOT standard ENU!)
    // PX4 quaternion: [w, x, y, z] (Hamilton convention)
    // NED: X-North, Y-East, Z-Down → Your custom: X-North, Y=-East, Z=-Down (up)
    ...
    q_enu.w = q_w;
    q_enu.x = q_x;    // NED X → Your X (roll)
    q_enu.y = -q_y;   // NED Y → Your Y (pitch), negate to fix nose up/down direction
    q_enu.z = -q_z;   // NED Z (Down) → Your Z (Up), negate for yaw
}
...
{
    // For quadrotor: Always publish odom->base_link when we have attitude
    // This ensures vehicle is always visible in RViz, even during position estimation issues
...
void Px4ToRosConverter::publish_odom_tf_with_timestamp(const builtin_interfaces::msg::Time& timestamp)
{
    ...
    // Set header - USE SYNCHRONIZED TIMESTAMP
    ...
    if (position_received_) {
        ...
        odom_tf.transform.translation.x = latest_position_.x;      // North -> Your X
        odom_tf.transform.translation.y = -latest_position_.y;     // East -> Your Y (negate)
        odom_tf.transform.translation.z = -latest_position_.z;     // Down -> Your Z (negate, up)
    } else {
    ...
    // Set rotation (always from attitude - this is the key for quadrotor visibility)
...
// Legacy timer-based functions (DISABLED but kept for compatibility)
void Px4ToRosConverter::publish_odom_tf()
{
    // DISABLED: TF now published in sync with pose
    // This function kept for timer compatibility only
    return;
}

void Px4ToRosConverter::publish_map_tf()
{
    // DISABLED: TF now published in sync with pose  
    // This function kept for timer compatibility only
    return;
